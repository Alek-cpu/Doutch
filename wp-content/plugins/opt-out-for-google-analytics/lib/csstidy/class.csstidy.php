	...
	class csstidy {
		...
		public function _add_token( $type, $data, $do = false ) {
			if ( $this->get_cfg( 'preserve_css' ) || $do ) {
				...
				if ( $type === AT_START and count( $this->tokens ) and $last = end( $this->tokens ) and $last[0] === AT_END and $last[1] === trim( $data ) ) {
		...
		public function css_new_media_section( $current_media, $new_media, $at_root = false ) {
			...
			if ( $current_media && ! $at_root // numeric $current_media means DEFAULT_AT or inc
			     && ! is_numeric( $current_media ) && strncmp( $this->get_cfg( 'css_level' ), 'CSS3', 4 ) == 0 ) {
		...
		public function is_important( &$value ) {
			return ( strpos( $value, '!' ) !== false); // quick test AND ! strcasecmp( substr( str_replace( $this->data['csstidy']['whitespace'], '', $value ), - 10, 10 ), '!important' ) );
		}
