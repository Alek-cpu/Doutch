...
#				) as attributes ON attributes.virtuemart_product_id = a.virtuemart_product_id
	...
	public function getProductList($parent, $remote_category) {
		...
		foreach ($this->products as $index => $prod) {
			...
			$item->attributes['link'] = $amwcore->siteHost . 'index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=' . $prod->product_id;
			...
			$item->imgurls = array();
			if ($media) {
				$item->attributes['feature_imgurl'] =  $amwcore->siteHost . $media[0]->file_url;
				foreach ($media as $fileIndex => $file)
					if ($fileIndex > 0)
						$item->imgurls[] = $amwcore->siteHost . $file->file_url;
			}
