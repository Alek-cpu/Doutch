...
class EbayMessagesTable extends WP_List_Table {
    ...
    function column_details($item){
        ...
        //Build row actions
        $actions = array(
            'view_ebay_message_details' => sprintf('<a href="?page=%s&action=%s&ebay_message=%s&width=600&height=470" class="thickbox">%s</a>',$page,'view_ebay_message_details',$item['id'],__( 'Details', 'wp-lister-for-ebay' )),
            'reply_on_ebay' => sprintf('<a href="%s" target="_blank">%s</a>',$item['response_url'],__( 'Reply on eBay', 'wp-lister-for-ebay' )),
        );
    ...
    function column_CompleteStatus($item){
        ...
        switch( $item['CompleteStatus'] ){
            case 'Unread':
                $color = 'darkorange';
                $value = __( 'Unread', 'wp-lister-for-ebay' );
				break;
            case 'Read':
                $color = 'green';
                $value = __( 'Read', 'wp-lister-for-ebay' );
				break;
            default:
                $color = 'black';
    ...
    function get_columns(){
        $columns = array(
            ...
            'received_date'     => __( 'Received at', 'wp-lister-for-ebay' ),
            // 'sender'            => __( 'Sender', 'wp-lister-for-ebay' ),
            'details'           => __( 'Subject', 'wp-lister-for-ebay' ),
            // 'subject'           => __( 'Subject', 'wp-lister-for-ebay' ),
            // 'item_id'  			=> __( 'eBay ID', 'wp-lister-for-ebay' ),
            'item_title'  		=> __( 'Product', 'wp-lister-for-ebay' ),
            // 'flag_read'         => '&nbsp;', // __( 'Read', 'wp-lister-for-ebay' ),
            // 'message_id'        => __( 'Message ID', 'wp-lister-for-ebay' ),
            // 'status'		 	=> __( 'Status', 'wp-lister-for-ebay' ),
            // 'expiration_date'	=> __( 'Expires at', 'wp-lister-for-ebay' ),
    ...
    function get_bulk_actions() {
        $actions = array(
            'wple_update_messages' => __( 'Update selected messages from eBay', 'wp-lister-for-ebay' ),
            'wple_delete_messages' => __( 'Delete selected messages', 'wp-lister-for-ebay' )
        );
    ...
    function get_views(){
        $views    = array();
        $current  = ( !empty($_REQUEST['message_status']) ? wple_clean($_REQUEST['message_status']) : 'all');
        ...
        $views['Read'] = "<a href='{$Read_url}' {$class} >".__( 'Read', 'wp-lister-for-ebay' )."</a>";
        ...
        $views['Unread'] = "<a href='{$Unread_url}' {$class} >".__( 'Unread', 'wp-lister-for-ebay' )."</a>";
