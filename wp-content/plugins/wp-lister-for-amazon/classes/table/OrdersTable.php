    ...
    function column_total($item){
        ...
        foreach ( $history as $record ) {
            if ( $record->action == 'reduce_stock' ) {
                ...
                $qty_purchased = isset( $record->details['qty_purchased'] ) ? $record->details['qty_purchased'] : '?';
                $tipmsgs      .= 'Stock level for product #'.$item['post_id'].' has been reduced by '.$qty_purchased.' unit(s).' . '<br>';
            }
        } // each history record

        if ( $tipmsgs ) {
            $img_url  = WPLA_URL . 'img/icon-success.png';
