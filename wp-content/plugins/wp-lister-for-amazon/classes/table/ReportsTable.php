...
class WPLA_ReportsTable extends WP_List_Table {
    ...
    function column_details($item){
        ...
        $actions = array(
            // 'view_amazon_report_details' => sprintf('<a href="?page=%s&action=%s&amazon_report=%s&width=600&height=470" class="thickbox">%s</a>',$page,'view_amazon_report_details',$item['id'],__( 'Details', 'wp-lister-for-amazon' )),
            'view_amazon_report_details' => sprintf('<a href="?page=%s&action=%s&amazon_report=%s" target="_blank">%s</a>',$page,'view_amazon_report_details',$item['id'],__( 'Details', 'wp-lister-for-amazon' )),
            'view_amazon_report_details' => sprintf('<a href="admin-ajax.php?action=%s&rrid=%s&sig=%s" target="_blank">%s</a>','wpla_report_details',$item['ReportRequestId'],$signature,__( 'Details', 'wp-lister-for-amazon' )),
            'load_report_from_amazon'    => sprintf('<a href="?page=%s&action=%s&amazon_report=%s&_wpnonce=%s">%s</a>',$page,'load_report_from_amazon',$item['id'], wp_create_nonce( 'wpla_load_report' ), __( 'Load', 'wp-lister-for-amazon' )),
            'process_amazon_report'      => sprintf('<a href="?page=%s&mode=%s&report_id=%s&step=2&_wpnonce=%s">%s</a>','wpla-import','inventory',$item['id'], wp_create_nonce( 'wpla_process_report' ), __( 'Process Report', 'wp-lister-for-amazon' )),
            'process_fba_report'         => sprintf('<a href="?page=%s&action=%s&amazon_report=%s&_wpnonce=%s" data-id="%s">%s</a>',$page,'process_amazon_report',$item['id'], wp_create_nonce( 'wpla_process_report' ), $item['id'], __( 'Process FBA Report', 'wp-lister-for-amazon' )),
            'process_fba_shipment_report' => sprintf('<a href="?page=%s&action=%s&amazon_report=%s&_wpnonce=%s" data-id="%s">%s</a>',$page,'process_fba_shipment_report',$item['id'],wp_create_nonce( 'wpla_process_fba_report' ), $item['id'],  __( 'Process Report', 'wp-lister-for-amazon' )),
            'process_fba_inv_age_report' => sprintf('<a href="?page=%s&action=%s&amazon_report=%s&_wpnonce=%s" data-id="%s">%s</a>',$page,'process_amazon_report',$item['id'], wp_create_nonce( 'wpla_process_report' ), $item['id'], __( 'Process Report', 'wp-lister-for-amazon' )),
            'process_quality_report'     => sprintf('<a href="?page=%s&action=%s&amazon_report=%s&_wpnonce=%s" data-id="%s">%s</a>',$page,'process_amazon_report',$item['id'], wp_create_nonce( 'wpla_process_report' ), $item['id'], __( 'Process Quality Report', 'wp-lister-for-amazon' )),
        ...
        if ( !in_array( $item['ReportType'], array( '_GET_AFN_INVENTORY_DATA_', '_GET_FBA_MYI_UNSUPPRESSED_INVENTORY_DATA_' ) ) ) {
            unset( $actions['process_fba_report'] );  
        }
    ...
    function column_account_id($item){
        ...
        if ( $account ) {
            ...
            return __('No account', 'wp-lister-for-amazon' );
        }
    ...
    function column_CompletedDate($item){
        ...
        return sprintf('%1$s <br><span style="color:silver">%2$s</span>',
            /*$1%s*/ get_date_from_gmt( $item['CompletedDate'], get_option( 'date_format' ) .' H:i' ),
    ...
    function column_status($item){
        ...
        switch( $item['ReportProcessingStatus'] ){
            ...
            case '_DONE_':
                $color = 'green';
                $value = __( 'Completed', 'wp-lister-for-amazon' );
                break;
            case '_CANCELLED_':
                $color = 'gray';
    ...
    function get_columns(){
        $columns = array(
            ...
            'ReportRequestId'   => __( 'Report ID', 'wp-lister-for-amazon' ),
            // 'ReportType'        => __( 'ReportType', 'wp-lister-for-amazon' ),
            'details'           => __( 'Type', 'wp-lister-for-amazon' ),
            'SubmittedDate'     => __( 'Submitted at', 'wp-lister-for-amazon' ),
            'CompletedDate'	    => __( 'Completed at', 'wp-lister-for-amazon' ),
    ...
    function get_bulk_actions() {
        $actions = array(
            'update_amazon_report' 	=> __( 'Update selected reports from Amazon', 'wp-lister-for-amazon' ),
            'delete_amazon_report'  => __( 'Delete selected reports', 'wp-lister-for-amazon' )
        );
        return $actions;
    }
    ...
    function get_views(){
       $views    = array();
       $current  = ( !empty($_REQUEST['report_status']) ? wpla_clean($_REQUEST['report_status']) : 'all');
       ...
       $views['_DONE_'] = "<a href='{$_DONE__url}' {$class} >".__( 'Done', 'wp-lister-for-amazon' )."</a>";
       ...
       $views['_DONE_NO_DATA_'] = "<a href='{$_DONE_NO_DATA__url}' {$class} >".__( 'No results', 'wp-lister-for-amazon' )."</a>";
       ...
       $views['_SUBMITTED_'] = "<a href='{$_SUBMITTED__url}' {$class} >".__( 'Submitted', 'wp-lister-for-amazon' )."</a>";
       ...
       $views['_CANCELLED_'] = "<a href='{$_CANCELLED__url}' {$class} >".__( 'Cancelled', 'wp-lister-for-amazon' )."</a>";
