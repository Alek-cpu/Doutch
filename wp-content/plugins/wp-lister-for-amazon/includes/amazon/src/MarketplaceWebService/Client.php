...
class MarketplaceWebService_Client implements MarketplaceWebService_Interface
{
      ...
      do {
        try {
          ...
          switch ($httpStatus) {
            ...
            // ***** BEGIN MWSWPL PATCH *****
            // Amazon returns http code 503 for RequestThrottled which can be misunderstood by load balancers
            // A better choice is code 429 (Too Many Requests), which WPLA should understand as well 
            // in case a load balancer setup would rewrite status codes for throttled requests.
            case 429:
            // ***** END MWSWPL PATCH *****
            case 500:
          	case 503:
          ...
          require_once ('MarketplaceWebService/Exception.php');
            ...
            if ( isset( $response ) && $response['Status'] == 403 && strpos( $response['ResponseBody'], 'captcha-bypass' ) !== false ) {
                // try to extract the IP
                preg_match( '/Your IP<\/span>\: ([^<\/span>]+)<\/span>/', $response['ResponseBody'], $matches );

                $error = 'Your request is being blocked by Cloudflare possibly due to your IP being blacklisted.';
                if ( isset( $matches[1] ) ) {
                    $error .= '  View your IP in <a href="#" target="_blank">Project Honey Pot</a>';
                }
            }
            ...
            wpla_show_message( $actionName.' request failed with error: '.$error,'error');
            // ***** END MWSWPL PATCH *****
  ...
  private function getParsedHeader($parsedHeader, $key) {
    // ***** BEGIN MWSWPL PATCH *****
    if ( $key = 'Content-MD5' ) {
      if ( array_key_exists( strtolower('X-WPL-Content-MD5'), $parsedHeader ) ) {
        $key = 'X-WPL-Content-MD5'; // original Content-MD5 header is not relayed by all proxies
      }
    }
    // ***** END MWSWPL PATCH *****
